...
#include <fstream> // For saving scores.
#include <chrono> // For ↑
#include <iomanip> // For ↑
...
int main()
{
    ...
    if(!musicTrack1.openFromFile("assets/music/OST_Track_1.wav")) {
        return EXIT_FAILURE;
    }
    musicTrack1.play();
    ...
    sf::Clock ScoreClock; // Clock to track player score.
    ...
    while (app.isOpen())
    {
        ...
        score = score + ScoreClock.getElapsedTime().asSeconds();
        ...
        for (auto& enemy : enemies) {
            ...
            if (eBounds.intersects(playerBounds) && enemy.health > 0) {
                playerHealth -= 5;  // Damage the player.
                score = score + 1500;
        ...
        if (fpsdt == 10) {
            fpsdt = 0;
            fpstext.setString("FPS: " + std::to_string(static_cast<int>(fps)) + "\nX: " + std::to_string(playex) + "\n" + "Y: " + std::to_string(playey));
        }
        ...
        // Draw Score.
        ScoreText.setPosition(0, windowy - 40);
        app.draw(ScoreText);

        // Draw the player sprite.
        app.draw(sprite);
        HealthText.setPosition(0, windowy - 27);
        app.draw(HealthText);
        // Death Text.
        if (playerIsDead) {
            musicIsPlaying = false;
            ...
            std::fstream file("assets/scores/scores.md", std::ios::in | std::ios::out | std::ios::app);
            if (file.is_open()) {
                file << std::put_time(std::localtime(&time), "%Y-%m-%d %H:%M:%S") << " Score: " << std::to_string(score) << "\n";
                file.close();
            } else {
                app.close();
            }
            app.clear(sf::Color::Black);
            ScoreText.setPosition(windowx/2 - 120, windowy/2-50);
